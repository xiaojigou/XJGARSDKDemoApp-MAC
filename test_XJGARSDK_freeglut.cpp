...
//int		g_iCamImgWidth = 640;
//int		g_iCamImgHeight = 480;
...
int		g_timvInterval = 33;
float	g_fTargetIdleTime = 1.f;
...
void initCVEnvironment()
{
	...
	mCamera.set(cv::CAP_PROP_FRAME_WIDTH, g_iCamImgWidth);
	mCamera.set(cv::CAP_PROP_FRAME_HEIGHT, g_iCamImgHeight);
	mCamera >> Image;
	...
	cv::namedWindow("controlwindows", cv::WINDOW_AUTOSIZE);
	cv::Mat emptyImg;
	...
	cv::createTrackbar("大眼", "controlwindows", &g_eyeParams_, 100, onTrackbarSlide);
	cv::createTrackbar("瘦脸", "controlwindows", &g_chinParams_, 100, onTrackbarSlide);
	cv::createTrackbar("红润", "controlwindows", &g_iRedFaceLevel_, 100, onTrackbarSlide);
	cv::createTrackbar("美白", "controlwindows", &g_WhiteSkinLevel_, 100, onTrackbarSlide);
	cv::createTrackbar("磨皮", "controlwindows", &g_SmoothSkinLevel_, 100, onTrackbarSlide);
...
void Draw(void)
{
	...
	if (isCaptureDone(device))
	{
		...
		float tmToSleep = g_fTargetIdleTime - float(time2 - time1 + 1);
		if (tmToSleep > 5.f)
...
#define XJGARSDKGetTargetResultImgAndLandMarks_TEST
            ...
            cv::cvtColor(targetResultImg, targetResultImg, cv::COLOR_RGB2BGR);
	...
	extern void onTrackbarSlide(int pos,void* p);
